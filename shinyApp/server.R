...
removeCodes = readRDS("./data/codes2Remove.Rds")
...
shinyServer(function(input, output) {
  ...
  output$predictedWordMain <- renderText({
    ...
    predictedWord(input$userSentence, k = 5 )$words[1]
  })
  ...
  prediction = reactive({
    ...
    data.table(word = predList$words[1:k], probability = predList$probs[1:k])
  ...
  output$kText = renderText(
    paste0(
      ...
      #input$numPredicted, 
      5,
      " predicted words with their probabilities:", 
      collapse = "")
  )
